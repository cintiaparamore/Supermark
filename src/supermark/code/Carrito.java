...
public class Carrito {
	...
	public void agregar(ProductoCarrito nuevo) {
		...
		// }
		// else System.out.println("�Lo siento! pero excede los 30 articulos por compra.
		// Vuelva a realizar otra compra");
	...
	public void graba_detalle() {
		...
		// Obtengo en la variable: ultimo_id_ventas, el último ID grabado en la tabla
		// ventas
		...
		for (int i = 0; i < this.productos.size(); i++) {
			...
			String sql_insertar_detalle = "INSERT INTO detalle_ventas (id_ventas, codigo, cantidad, subtotal) VALUES ("
					+ ultimo_id_ventas + "," + codip + "," + cantip + "," + subtotalp + ")";
			...
			nuevo_stock = this.productos.get(i).getStock() - this.productos.get(i).getCantidad();

			String sql_actualiza_stock = "UPDATE productos SET stock =" + nuevo_stock + " WHERE codigo = " + codip;

			try {
				stmt.executeUpdate(sql_actualiza_stock);
	...
	public void mostrar() {
		...
		if (this.productos.size() != 0) {
			...
			System.out.println("Total de articulos en carrito: " + cantidadcarrito);
